...
class icmphdr(object):
    ...
    def assemble(self):
        ...
        cksum = self.checksum(part1 + b"\x00\x00" + part2 + self.data.encode())
        cksum = struct.pack("!H", cksum)
        self._raw = part1 + cksum + part2 + self.data.encode()
        return self._raw
...
def geoip_lookup(ip):
    ...
    url = "https://api.ipgeolocationapi.com/geolocate/%s" % ip
    data_geo = yield requests.get(url)
    try:
        json_data = data_geo.json()
        return "({}, {})".format(json_data['nationality'], json_data['region'])
        #defer.returnValue(json_data['nationality'])
    except:
